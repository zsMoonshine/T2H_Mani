...
parser.add_argument('--config', type=str, default='config/PEDES_user_input.yaml', help='Path to the config file.')
parser.add_argument('--checkpoint_path', type=str, default='./pretrain', help="checkpoint path")
...
if __name__ == '__main__':
    ...
    for it, (data_a, data_b) in enumerate(zip(test_loader_a, test_loader_b)):
        ...
        with torch.no_grad():
            test_image_outputs = trainer.test_sample_user_input(images_a, images_b, pose_a, pose_b, caps_a, caps_len_a,
                                                                caps_b, caps_len_b, ixtoword_test, wordtoix_test)

        result_a = test_image_outputs[2]
        result_b = test_image_outputs[5]
        img_all_a = untransformTensor(result_a.cpu())
        img_all_b = untransformTensor(result_b.cpu())

        img_all_a_real = untransformTensor(images_a.cpu())
        img_all_b_real = untransformTensor(images_b.cpu())

        result_a_pose = test_image_outputs[1]
        result_b_pose = test_image_outputs[4]
        img_all_a_pose = untransformTensor(result_a_pose.cpu())
        img_all_b_pose = untransformTensor(result_b_pose.cpu())

        result_a_nlp = test_image_outputs[6]
        result_b_nlp = test_image_outputs[7]
        img_all_a_nlp = untransformTensor(result_a_nlp.cpu())
        img_all_b_nlp = untransformTensor(result_b_nlp.cpu())
        writeTensor('%s/%s.jpg' % (image_directory_test, im_path_a_final), img_all_a, nRow=images_a.shape[0])
